export default function XxlBanner() {
  return (
    <div className="max-w-screen relative flex h-[854px] items-center justify-center bg-slate-300">
      {/* Icon */}
      <div className="absolute left-[230px] top-[225px] z-40 bg-red-300">
        <img
          src="data:image/png;base64,"
          alt=""
        />
      </div>
      {/* Text */}
      <div className="absolute left-[260px] top-[245px] z-50 bg-yellow-300">
        <div className="absolute -mt-[1000px]"></div>
        <h3 className="text-3xl font-bold"> HEY !</h3>
        <h3 className="mb-[40px] text-3xl font-bold">ENJOY YOUR COFFEE TIME</h3>
        <p className="font-[20px]">100% ARABICA COFFEE</p>
        <p className="mb-[40px] font-[20px]">FRESHLY ROASTED &amp; BREWED</p>
        <h3 className="text-3xl font-bold">@TOMORO</h3>
      </div>
      {/* Background */}
      <img
        src="https://www.tomoro-coffee.com/img/icon1.10800737.png"
        alt=""
        className="absolute right-[263px] z-10 flex h-[647px] w-[1105px] justify-center bg-blue-300"
      />
    </div>
  );
}
